...
const Ventas = () => {
  ...
  const generarPDFs = async (ventaId) => {
    ...
    doc.text(`${ventaSeleccionada.created_at}`, 180, 50, null, null, 'right');
    ...
    for (const detalleItem of ventaSeleccionada.detalle) {
       try {
         ...
         doc.text(`${nombreProducto}`, 180, 60, null, null, 'right');
         doc.line(20, 63, 180, 63);
         doc.text('Precio:  ', 20, 70);
         doc.text(`S/.${costoProducto}`, 180, 70, null, null, 'right');
         doc.line(20, 73, 180, 73);
       } catch (error) {
    ...
    doc.text('cantidad: ', 20, 80);
    doc.text(`${ventaSeleccionada.numero}`, 180, 80, null, null, 'right');
    doc.line(20, 83, 180, 83);
    doc.text('Cliente ID: ', 20, 90);
    doc.text(`${ventaSeleccionada.clienteId}`, 180, 90, null, null, 'right');
    doc.line(20, 93, 180, 93);
    doc.text('Subtotal:  ', 20, 100);
    doc.text(`S/.${subtotal.toFixed(2)}`, 180, 100, null, null, 'right');
    doc.line(20, 103, 180, 103);
    doc.text('IGV (18%):  ', 20, 110);
    doc.text(`S/.${igv.toFixed(2)}`, 180, 110, null, null, 'right');
    doc.line(20, 113, 180, 113);
    doc.text('Total:  ', 20, 120);
    doc.text(`S/.${total.toFixed(2)}`, 180, 120, null, null, 'right');
